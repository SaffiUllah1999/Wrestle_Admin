...
export default function Products() {
    ...
    const Get_Products = () => {
      ...
      commonDataService
        .fetchData(SERVICE_ROUTE.GET_ECOMMER_PRODUCTS)
        .then((res) => {
          setDataset(res?.data);
        })
        .catch((error) => {
    ...
    const Add_Article = () => {
      setLoading(true); // Set loading to true
      commonDataService
        .executeApiCall(SERVICE_ROUTE.UPLOAD_ECOMMER_PRODUCTS, newArticle)
        .then((res) => {
          setDataset((prev) => [...prev, res?.data]);
          setModalOpen(false);
          setNewArticle({ name: "", price: "", image: "", category:""});
        })
        .catch((error) => {
          console.log(error);
        })
          ...
          {modalOpen && (
            <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
              <div className="bg-white p-5 rounded-lg shadow-lg">
                <h2 className="text-xl mb-4">Add Products</h2>
                <input
                  type="text"
                  placeholder="Name"
